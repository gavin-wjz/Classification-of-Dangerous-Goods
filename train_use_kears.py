...
class Model:
    ...
    def save_model(self, file_path):
        if not os.path.isdir(file_path):
            os.mkdir(file_path)
            print(file_path)
        self.model.save(file_path+'/model.h5')
...
if __name__ == '__main__':
    dataset = Dataset('H:/data1/')
    dataset.load()
    '''
    model = Model()
    model.build_model(dataset)
    model.train(dataset)
    model.save_model('H:/data/model/model.h5')
    '''
    # 评估模型
    model = Model()
    model.load_model('H:/data/model/model.h5')
    model.evaluate(dataset)
    #img=cv2.imread("H:/picture/23.jpg")
    #model.predict(img)